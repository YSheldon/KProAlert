...
def make_plan(client, database=None, cli=None, base=None, table=None,
              ...
              native_entry=None,native_entry_sha256=None,operations_table=None,source_egress_table=None):
    if client not in ('codex', 'workbuddy', 'cursor', 'grok', 'zcode', 'generic'):
        raise ValueError('Unsupported assistant')
    onboarding_only = not any((database, cli, base, table, operations_table, source_egress_table))
    server = build_config(database, cli, base, table, collector_health, endpoint_device_id,
                          ...
                          operations_table=operations_table,source_egress_table=source_egress_table)['mcpServers']['kpro-alerts']
    command = []
...
def main():
    ...
    plan = make_plan(args.client,args.database,args.cli,args.base,args.table,
                     ...
                     endpoint_device_id=args.endpoint_device_id,operations_table=args.operations_table,source_egress_table=args.source_egress_table)
    if args.apply:
